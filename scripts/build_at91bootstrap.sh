...
git -C ${at91boot_dir} checkout v3.8.10 -b tmp
echo "building at91bootstrap"
make -C ${at91boot_dir} ARCH=arm CROSS_COMPILE=${CC} distclean
make -C ${at91boot_dir} ARCH=arm CROSS_COMPILE=${CC} sama5d27_som1_eksd1_uboot_defconfig
make -C ${at91boot_dir} ARCH=arm CROSS_COMPILE=${CC}

# copy the built bin to the output
cp -v ${at91boot_dir}/binaries/sama5d27_som1_ek-sdcardboot-uboot-3.8.10.bin ${at91boot_bin}/BOOT.BIN
